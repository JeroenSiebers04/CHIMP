...
def get_serving_api_availability():
    ...
    try:
        ...
        write_file(f"\nCould not connect to serving API at {ip}: {e}\n")

    if response.text != "pong":
        print(f"Unexpected response from serving API at {ip}: {response.text}\n")
        write_file(f"\nUnexpected response from serving API at {ip}: {response.text}\n")
    else:
        if response.status_code == 200:
            print(f"The serving is available at {ip}. Returned http-code is as expected: {response.status_code}\n")
            write_file(f"\nThe serving is available at {ip}. Returned http-code is as expected: {response.status_code}\n")
        else:
            print(f"Got an unexpected http-code from {ip}. Status code should be 200, but is {response.status_code}\n")
            write_file(f"Got an unexpected http-code from {ip}. Status code should be 200, but is {response.status_code}\n")
